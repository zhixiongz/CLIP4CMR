...
class ViLT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=32, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=True, qk_scale=None, representation_size=None, distilled=False,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., embed_layer=PatchEmbed, norm_layer=None,
                 act_layer=None, weight_init=''):
        """
    ...
    def forward_features(self, x):
        # x = self.pos_drop(x + self.pos_embed)  # 和预训练参数的size不匹配
        x = self.blocks(x)
        x = self.norm(x)
        if self.dist_token is None:
    ...
    def forward(self, x):
        x = self.forward_features(x)   # torch.size([1, 768])
        if self.head_dist is not None:
            x, x_dist = self.head(x[0]), self.head_dist(x[1])  # x must be a tuple
            if self.training and not torch.jit.is_scripting():
                # during inference, return the average of both classifier predictions
